...
const API_URL = apiUrl('/api/adsblol/mil');
...
async function _backfillTrail(icao24, token, oldestFixEpochSec) {
  ...
  let trace = null;
  try {
    const response = await fetch(apiUrl('/api/adsblol/trace?hex=' + encodeURIComponent(icao24)), {
...
const militaryFlightsLayer = {
  ...
  init(viewer) {
    ...
    if (!_preloadModel) {
      ...
      Cesium.Model.fromGltfAsync({ url: withBase(JET_MODEL_URL), asynchronous: false })
        .then((m) => {
